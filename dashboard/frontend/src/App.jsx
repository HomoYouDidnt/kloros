  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg lg:col-span-2">
        <h2 className="text-lg font-semibold mb-4">📝 Recent Conversation Turns</h2>
            ...
            {metaState.recent_turns.map((turn, idx) => (
              <div key={idx} className={`p-3 rounded ${turn.role === 'user' ? 'bg-blue-900/20 border-l-4 border-blue-400' : 'bg-green-900/20 border-l-4 border-green-400'}`}>
                <div className="flex justify-between items-start mb-1">
                  <span className="font-semibold text-sm">{turn.role === 'user' ? '👤 User' : '🤖 KLoROS'}</span>
                  {turn.timestamp && (
      ...
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
        <h2 className="text-lg font-semibold mb-4">🕸️ Topic & Entity Network</h2>
...
function ConsciousnessTab({ metaState, emotionalTrajectory }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* Core Affect State */}
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
        <h2 className="text-lg font-semibold mb-4">🎭 Core Affect</h2>
        <div className="space-y-3">
      ...
      {emotionalTrajectory.length > 5 && (
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
          <h2 className="text-lg font-semibold mb-4">📈 Emotional Trajectory</h2>
          <ResponsiveContainer width="100%" height={250}>
            <ScatterChart>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
      ...
      {Object.keys(primaryEmotions).length > 0 && (
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
          <h2 className="text-lg font-semibold mb-4">💫 Primary Emotions (Panksepp)</h2>
          <ResponsiveContainer width="100%" height={250}>
      ...
      {homeostatic.length > 0 && (
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
          <h2 className="text-lg font-semibold mb-4">⚖️ Homeostatic Variables</h2>
          <div className="space-y-3">
...
function MemoryTab({ metaState }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {/* Memory Statistics */}
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
        <h2 className="text-lg font-semibold mb-4">📊 Memory Statistics</h2>
        <div className="space-y-4">
          <div>
      ...
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg lg:col-span-2">
        <h2 className="text-lg font-semibold mb-4">💡 Recent Memory Insights & Reflections</h2>
...
function CuriosityTab() {
  ...
  const [remoteLLMConfig, setRemoteLLMConfig] = useState({ enabled: false, selected_model: 'qwen2.5:72b', server_url: 'http://100.67.244.66:11434' });
  const [remoteLLMStatus, setRemoteLLMStatus] = useState({ reachable: false, available_models: [], error: null });
  const [updatingRemoteLLM, setUpdatingRemoteLLM] = useState(false);
  const [testingRemoteLLM, setTestingRemoteLLM] = useState(false);
  const [testResult, setTestResult] = useState(null);

  useEffect(() => {
    const fetchCuriosityData = async () => {
      try {
        const [curiosityRes, dialogueRes, tracesRes, llmConfigRes, remoteLLMConfigRes, remoteLLMStatusRes] = await Promise.all([
          ...
          fetch(`${API_URL}/api/curiosity/remote-llm-config`),
          fetch(`${API_URL}/api/curiosity/remote-status`)
        ]);
        ...
        if (remoteLLMConfigRes.ok) {
          const data = await remoteLLMConfigRes.json();
          setRemoteLLMConfig(data);
        }
        
        if (remoteLLMStatusRes.ok) {
          const data = await remoteLLMStatusRes.json();
          setRemoteLLMStatus(data);
        }
      } catch (error) {
  ...
  const handleToggleRemoteLLM = async () => {
    setUpdatingRemoteLLM(true);
    try {
      const response = await fetch(`${API_URL}/api/curiosity/remote-llm-config?enabled=${!remoteLLMConfig.enabled}`, {
        method: 'POST',
      });
      if (response.ok) {
        const updated = await response.json();
        setRemoteLLMConfig(updated);
      }
    } catch (error) {
      console.error('[curiosity] Error toggling remote LLM:', error);
    } finally {
      setUpdatingRemoteLLM(false);
    }
  };

  const handleTestQuery = async () => {
    setTestingRemoteLLM(true);
    setTestResult(null);
    try {
      const response = await fetch(`${API_URL}/api/curiosity/remote-query`, {
        method: 'POST',
      ...
      console.error('[curiosity] Error testing remote LLM:', error);
      setTestResult({ success: false, error: error.message });
    } finally {
      setTestingRemoteLLM(false);
    }
  };
  ...
  const getDialogueTypeColor = (type) => {
    const colors = {
      ...
      concern: 'border-orange-400 bg-orange-900/20',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-400">
                Access to Ollama server on ALTIMITOS (Tailscale: {remoteLLMConfig.server_url})
              </p>
            </div>
            <button
              onClick={handleToggleRemoteLLM}
              disabled={updatingRemoteLLM}
              className={`ml-4 relative inline-flex h-8 w-16 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:ring-offset-gray-800 ${
                remoteLLMConfig.enabled ? 'bg-cyan-600' : 'bg-gray-600'
              } ${updatingRemoteLLM ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
            >
              <span
                className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${
                  remoteLLMConfig.enabled ? 'translate-x-9' : 'translate-x-1'
                }`}
              />
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-gray-700/30 rounded p-3">
              <p className="text-xs text-gray-400 mb-1">Available Models: {remoteLLMStatus.available_models.length}</p>
              <p className="text-xs text-gray-500">KLoROS will automatically select the best model for each query</p>
            </div>
            ...
            <div>
              ...
              <button
                ...
                disabled={testingRemoteLLM || !remoteLLMStatus.reachable || !remoteLLMConfig.enabled}
                className="w-full px-4 py-2 bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded font-medium transition-colors"
              >
                {testingRemoteLLM ? 'Testing...' : 'Send Test Query'}
              </button>
              {testResult && (
                <div className={`mt-2 p-2 rounded text-xs ${testResult.success ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
                  {testResult.success ? (
                    <div>
                      <div className="font-semibold">✓ Success!</div>
                      <div>Response: "{testResult.response}"</div>
                      <div className="text-gray-400 mt-1">Time: {(testResult.response_time_ms / 1000).toFixed(1)}s</div>
                    </div>
                  ) : (
      ...
      {/* Curiosity Overview */}
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
        <h2 className="text-lg font-semibold mb-4">🔍 Curiosity State</h2>
        {curiosityState ? (
          <div className="space-y-4">
            <div>
      ...
      {/* Internal Dialogue */}
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg lg:col-span-2">
        <h2 className="text-lg font-semibold mb-4">💭 Internal Dialogue</h2>
      ...
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg lg:col-span-2">
        <h2 className="text-lg font-semibold mb-4">🧩 Explainable AI Reasoning Traces</h2>
        {xaiTraces.length > 0 ? (
          <div className="space-y-3">
            {xaiTraces.map((trace) => (
  ...
  return (
    <div className="space-y-6">
      ...
      {resources.gpus && resources.gpus.length > 0 && (
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
          <h2 className="text-lg font-semibold mb-4">🎮 GPU Utilization</h2>
          <div className="space-y-4">
      ...
      {resourceHistory.length > 5 && (
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg lg:col-span-2">
          <h2 className="text-lg font-semibold mb-4">📉 Resource History (Last 5 Minutes)</h2>
