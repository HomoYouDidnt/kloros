...
from .repetition_prevention import RepetitionChecker
from .topic_tracker import TopicTracker
from .models import EventType, ContextRetrievalRequest, ContextRetrievalResult
from .autonomous_decay import start_autonomous_decay, stop_autonomous_decay
...
class MemoryEnhancedKLoROS:
    ...
    def __init__(self, kloros_instance):
        ...
        self.repetition_checker = RepetitionChecker(
            similarity_threshold=float(os.getenv("KLR_REPETITION_THRESHOLD", "0.75")),
            history_size=int(os.getenv("KLR_REPETITION_HISTORY_SIZE", "10"))
        )
        self.topic_tracker = TopicTracker(
            max_keywords=int(os.getenv("KLR_TOPIC_MAX_KEYWORDS", "50"))
        )
        ...
        # Start autonomous decay manager if enabled
        self.auto_start_decay = int(os.getenv("KLR_AUTO_START_DECAY", "1"))
        self.decay_manager = None

        if self.auto_start_decay and self.enable_memory:
            try:
                update_interval = int(os.getenv("KLR_DECAY_UPDATE_INTERVAL", "60"))
                self.decay_manager = start_autonomous_decay(
                    store=self.memory_store,
                    update_interval_minutes=update_interval
                )
                print(f"[memory] ✅ Started autonomous decay manager (updates every {update_interval} minutes)")
            except Exception as e:
                print(f"[memory] ⚠️ Failed to start autonomous decay manager: {e}")
    ...
    def _wrap_kloros_methods(self):
        ...
        # Wrap chat method (required)
        if hasattr(self.kloros, 'chat'):
            self._original_chat = self.kloros.chat
            self.kloros.chat = self._memory_enhanced_chat
            print("[memory] ✅ Wrapped chat method with memory enhancement")

        # Wrap conversation handler if it exists (voice mode)
        if hasattr(self.kloros, 'handle_conversation'):
        ...
        # Wrap wake word listener if it exists (voice mode)
        if hasattr(self.kloros, 'listen_for_wake_word'):
    ...
    def __getattr__(self, name):
        """Proxy attribute access to the wrapped KLoROS instance."""
        # Avoid infinite recursion by using object.__getattribute__
        try:
            kloros = object.__getattribute__(self, 'kloros')
            return getattr(kloros, name)
    ...
    def _memory_enhanced_handle_conversation(self):
        ...
        # Log conversation start and clear repetition/topic tracking
    ...
    def _memory_enhanced_chat(self, user_message: str) -> str:
        ...
        # Log user input and add to topic tracking
        ...
        # Context retrieval removed from wrapper layer to eliminate duplication
        # Backend layer (local_rag_backend.py) handles context retrieval directly
        ...
        try:
            ...
            is_repetitive, similar_response, similarity = self.repetition_checker.is_repetitive(response)

            if is_repetitive:
                # Log repetition warning
                self.memory_logger.log_event(
                    event_type=EventType.SYSTEM_NOTE,
                    content=f"Repetitive response detected (similarity: {similarity:.2f})",
                    metadata={
                        "response": response[:100],  # First 100 chars
                        "similar_to": similar_response[:100] if similar_response else None,
                        "similarity_score": similarity
                    }
                )
                print(f"[memory] ⚠️  Repetition detected: {similarity:.1%} similar to recent response")

            # Add response to repetition history and topic tracking
    ...
    def _format_context_for_prompt(self, context_result) -> str:
        ...
        if context_result.summaries:
            ...
            for summary in context_result.summaries:  # Use all summaries (up to max_context_summaries)
                context_parts.append(f"- {summary.summary_text}")
            context_parts.append("")  # Add blank line separator

        # Add relevant events with clear User/KLoROS labels
        if context_result.events:
            context_parts.append("Recent conversation turns:")
            for event in context_result.events:  # Use all events (up to max_context_events, now 20)
                event_type_str = event.event_type if isinstance(event.event_type, str) else event.event_type.value
                if event_type_str == "user_input":
                    context_parts.append(f"User: {event.content}")
                elif event_type_str == "llm_response":
                    context_parts.append(f"KLoROS: {event.content}")

        # Increased from 500 to 2000 chars for better conversation continuity
    ...
    def close(self):
        ...
        if self.enable_memory:
            # Stop autonomous decay manager if running
            if self.decay_manager is not None:
                try:
                    stop_autonomous_decay()
                    print("[memory] ✅ Stopped autonomous decay manager")
                except Exception as e:
                    print(f"[memory] ⚠️ Error stopping decay manager: {e}")
