...
class ConversationDomainEvaluator(DomainEvaluator):
    ...
    def run_probes(self, config: Dict[str, Any]) -> Dict[str, float]:
        ...
        Only generates WAV files for audio-dependent metrics. Text-based metrics
        are evaluated without synthesis to reduce resource usage.
        ...
        # Determine which metrics require audio synthesis
        needs_audio = self._check_if_audio_needed(config)

        try:
            if needs_audio:
                # Generate test speech only for audio-dependent metrics
                test_outputs = self._generate_test_speech(config)

                if not test_outputs:
                    logger.error("All TTS generation failed - returning zero fitness")
                    return {
                        'pronunciation_accuracy': 0.0,
                        'tone_similarity': 0.0,
                        'sarcasm_effectiveness': 0.0,
                        'naturalness_score': 0.0,
                        'speech_clarity': 0.0,
                        'personality_consistency': 1.0,
                    }

                # Audio-dependent metrics
                ...
                # Skip audio synthesis - use text-only evaluation
                logger.info("Skipping TTS synthesis - using text-based metrics only")
                metrics['pronunciation_accuracy'] = 0.75  # Neutral baseline
                metrics['tone_similarity'] = 0.75
                metrics['naturalness_score'] = 0.75
                metrics['speech_clarity'] = 0.75

            # Text-based metrics (never need audio)
            metrics['sarcasm_effectiveness'] = self._evaluate_sarcasm(None, config)
            metrics['personality_consistency'] = self._check_personality_drift(config)

            # Apply failure penalty if audio was attempted
            if needs_audio and self._tts_failures > 0:
                failure_penalty = 1.0 - (self._tts_failures * 0.25)
                for key in metrics:
                    if key != 'personality_consistency':
                        metrics[key] *= max(0.1, failure_penalty)

        finally:
            # Clean up test WAV files
            for wav_file in test_outputs:
                try:
                    if wav_file.exists():
                        wav_file.unlink()
                except Exception as e:
                    logger.warning(f"Failed to delete test WAV {wav_file}: {e}")

        return metrics

    def _check_if_audio_needed(self, config: Dict[str, Any]) -> bool:
        """Check if audio synthesis is needed for current parameters.

        Audio is only needed when TTS-specific parameters are being tuned.
        Text parameters (context turns, response length, etc.) don't need it.

        Returns:
            True if WAV generation needed, False otherwise
        """
        # Audio-dependent parameters
        audio_params = {
            'speaking_rate', 'pitch_variance', 'emphasis_strength',
            'pause_duration', 'phoneme_precision', 'consonant_strength',
            'vowel_length', 'tone_sharpness', 'monotone_factor'
        }

        # Check if any audio params are in the config
        return any(param in config for param in audio_params)
    ...
    def _generate_test_speech(self, config: Dict[str, Any]) -> List[Path]:
        ...
        try:
            ...
            for i, phrase in enumerate(test_phrases):
                # Use PID to avoid file collision in parallel experiments
                output_file = Path(f'/tmp/conversation_test_{i}_{os.getpid()}.wav')
                ...
                # Generate audio with timeout and error tracking
                try:
                    ...
                    if result.returncode == 0 and output_file.exists():
                        ...
                        failures += 1
                        stderr_msg = result.stderr.decode('utf-8', errors='ignore') if result.stderr else 'unknown'
                        logger.warning(f"TTS failed for phrase {i}: {stderr_msg[:200]}")

                except subprocess.TimeoutExpired:
                    failures += 1
                    logger.error(f"TTS timed out for phrase {i}")
                except Exception as e:
                    failures += 1
                    logger.error(f"TTS exception for phrase {i}: {e}")

        except Exception as e:
            logger.error(f"Test speech generation failed: {e}")

        # Store failure count for fitness penalty
        self._tts_failures = failures
        return outputs
    ...
    def _evaluate_sarcasm(self, outputs: Optional[List[Path]], config: Dict[str, Any]) -> float:
        """Evaluate sarcasm effectiveness.

        Can work with or without audio - uses config parameters directly.
        """
        ...
        # Combined score (no audio needed)
    ...
    def _measure_clarity(self, outputs: List[Path]) -> float:
        ...
        try:
            ...
            for output_file in outputs:
                ...
                # sox writes stats to stderr, so check both stdout and stderr
                output = result.stdout + result.stderr
                if 'RMS lev dB' in output:
                    # Parse RMS level
