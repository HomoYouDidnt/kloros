...
SAFETY (Nov 5, 2025): Integrated with ResourceGovernor to prevent
runaway resource consumption via disk checks, rate limiting, instance
caps, and circuit breaker protection.
"""
import os
import json
import shutil
...
# ResourceGovernor integration for safety
try:
    from src.governance.resource_governor import ResourceGovernor
    from src.kloros.orchestration import state_manager
    SAFETY_ENABLED = True
    logger.info("ResourceGovernor safety mechanisms enabled")
except ImportError as e:
    logger.warning(f"ResourceGovernor not available, safety checks disabled: {e}")
...
def spawn_instance(candidate: Dict[str, float], parent_id: Optional[str] = None,
    ...
    SAFETY: Enforces resource limits via ResourceGovernor before spawning.
    Will fail if disk space low, rate limit exceeded, instance cap reached,
    ...
    Raises:
        RuntimeError: If resource limits prevent spawning or spawn fails
    """
    # STEP 1: Cleanup old instances FIRST (before ResourceGovernor check)
    # This prevents deadlock: if instances > limit, cleanup runs and reduces count
    # so ResourceGovernor check can pass
    try:
        prune_old_instances()
        logger.info("Completed pre-spawn instance cleanup")
    except Exception as e:
        logger.warning(f"Pre-spawn cleanup failed (non-fatal): {e}")
    ...
    if SAFETY_ENABLED:
        ...
        if not can_spawn:
            error_msg = f"Spawn blocked by ResourceGovernor: {reason}"
            logger.error(error_msg)
            governor.record_spawn_attempt(success=False, reason=reason)
            raise RuntimeError(error_msg)

    # STEP 3: Acquire lock for atomic spawning (prevents race condition)
    lock = None
    if SAFETY_ENABLED:
        try:
            lock = state_manager.acquire("spica_spawner", ttl_s=300)
            logger.debug("Acquired spica_spawner lock")
        except RuntimeError as e:
            error_msg = f"Failed to acquire spawn lock: {e}"
            logger.error(error_msg)
            if SAFETY_ENABLED:
                governor = ResourceGovernor()
                governor.record_spawn_attempt(success=False, reason="lock_contention")
            raise RuntimeError(error_msg)
...
"""
        ...
        # SAFETY: Record successful spawn
        if SAFETY_ENABLED:
            governor = ResourceGovernor()
        ...
        # SAFETY: Record failed spawn
        if SAFETY_ENABLED:
            governor = ResourceGovernor()
            governor.record_spawn_attempt(success=False, reason=str(e))

        # Cleanup partial instance if it was created
        if instance_dir and instance_dir.exists():
            logger.warning(f"Cleaning up failed spawn: {instance_dir}")
            try:
                shutil.rmtree(instance_dir)
            except Exception as cleanup_error:
                logger.error(f"Failed to cleanup {instance_dir}: {cleanup_error}")

        raise

    finally:
        # SAFETY: Release lock
        if lock and SAFETY_ENABLED:
            try:
                state_manager.release(lock)
                logger.debug("Released spica_spawner lock")
            except Exception as release_error:
                logger.error(f"Failed to release lock: {release_error}")
