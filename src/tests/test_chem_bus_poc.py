...
from src.orchestration.core.signal_router import SignalRouter
...
def test_signal_router():
    ...
    sub = UMNSub("Q_ORPHANED_QUEUE", on_signal, ipc_path="/tmp/test_router.ipc")
    time.sleep(0.1)
...
def test_multi_subscriber():
    ...
    pub = UMNPub(ipc_path="/tmp/test_multi.ipc")
    time.sleep(0.1)

    # Simulate two zooids subscribing to same signal
    print("2. Spawning LatencyMonitor zooid...")
    sub1 = UMNSub("Q_LATENCY_SPIKE", make_handler("latency_monitor"), ipc_path="/tmp/test_multi.ipc")

    print("3. Spawning BackpressureController zooid...")
    sub2 = UMNSub("Q_LATENCY_SPIKE", make_handler("backpressure_controller"), ipc_path="/tmp/test_multi.ipc")
...
def main():
    ...
    if all_passed:
        print("\n🎉 All tests passed! Unus Mundus Network is functional.")
        print("\nThis proves:")
