...
class HybridSttBackend:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            vosk_model_dir: VOSK model directory (if vosk_model not provided)
            vosk_model: Pre-loaded vosk.Model instance (to share with wake word detection)
    ...
    def transcribe(
        ...
        # Step 2: Get Whisper result (accurate path) - runs in tandem for auto-correction
    ...
    def _apply_hybrid_logic(
        ...
        Enhanced with tighter acceptance rules to prevent low-quality transcripts.
        ...
        # Extract Whisper quality metrics from raw data
        whisper_raw = whisper_result.raw.get("whisper_result", {})
        segments = whisper_raw.get("segments", [])

        # Calculate average logprob and compression ratio
        avg_logprob = -999.0
        compression_ratio = 999.0
        if segments:
            logprobs = [seg.get("avg_logprob", -999.0) for seg in segments]
            compression_ratios = [seg.get("compression_ratio", 999.0) for seg in segments]
            avg_logprob = sum(logprobs) / len(logprobs) if logprobs else -999.0
            compression_ratio = sum(compression_ratios) / len(compression_ratios) if compression_ratios else 999.0

        # Tighter acceptance rules
        # Vosk acceptance: confidence ≥ 0.82 AND similarity ≥ 0.88
        vosk_acceptable = (vosk_result.confidence >= 0.82 and similarity >= 0.88)

        # Whisper acceptance: logprob ≥ -0.75 OR compression_ratio ≤ 2.5
        whisper_acceptable = (avg_logprob >= -0.75 or compression_ratio <= 2.5)

        # Decision logic
        if similarity >= self.correction_threshold:
            # Transcripts are similar - boost confidence and prefer higher confidence
            # But only if at least one passes acceptance rules
            if whisper_acceptable and whisper_result.confidence > vosk_result.confidence:
                result = self._create_boosted_result(whisper_result, similarity, "whisper_confidence")
                self.stats["whisper_wins"] += 1
            elif vosk_acceptable:
                result = self._create_boosted_result(vosk_result, similarity, "vosk_confidence")
                self.stats["vosk_wins"] += 1
            elif whisper_result.confidence > vosk_result.confidence:
                # Fallback: prefer higher confidence even if not acceptable
                result = self._create_boosted_result(whisper_result, similarity, "whisper_fallback")
                self.stats["whisper_wins"] += 1
            else:
                result = self._create_boosted_result(vosk_result, similarity, "vosk_fallback")
                self.stats["vosk_wins"] += 1
        else:
            # Transcripts differ significantly - apply correction logic
            if whisper_acceptable and whisper_result.confidence > self.confidence_boost_threshold:
                ...
                # Vosk is acceptable, prefer it for responsiveness
                result = vosk_result
                self.stats["vosk_wins"] += 1
            else:
                # Neither acceptable - prefer VOSK for speed (fallback)
                result = vosk_result
                self.stats["vosk_wins"] += 1
