#!/usr/bin/env python3
"""
Session Management Zooid for KLoROS Voice System (Phase 5)
...
Fail-Safe:
- If this zooid is dead, orchestrator operates in stateless mode (no conversation memory)
"""
...
class SessionZooid:
    """Session Management zooid - conversation history, state persistence, context truncation."""

    def __init__(self):
        self.zooid_name = "kloros-voice-session"
        ...
        # ChemBus publishers/subscribers (initialized in start())
        self.chem_pub: Optional[ChemPub] = None
        self.stt_sub: Optional[ChemSub] = None
        self.llm_sub: Optional[ChemSub] = None

        # Running state
        self.running = False

        print(f"[session] SessionZooid initialized (session_id={self.session_id}, max_entries={self.max_history_entries})")

    def start(self):
        """Initialize ChemBus connections and load persisted state."""
        print("[session] Starting Session Management zooid...")

        # Initialize ChemBus publisher
        try:
            self.chem_pub = ChemPub()
            print("[session] ChemBus publisher initialized")
        except Exception as e:
            print(f"[session] CRITICAL: Failed to initialize ChemPub: {e}")
            sys.exit(1)
        ...
        self.running = True
        print("[session] Session Management zooid ready")
    ...
    def shutdown(self):
        """Shutdown session zooid gracefully."""
        print("[session] Shutting down Session Management zooid...")
        ...
        self.running = False
        print("[session] Session Management zooid stopped")


def main():
    """Main entry point for Session Management zooid."""
    zooid = SessionZooid()
