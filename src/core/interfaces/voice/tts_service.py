#!/usr/bin/env python3
"""KLoROS Voice TTS Service - Text-to-speech synthesis.

This service handles:
- Convert text to speech using SmartTTSRouter (Piper + Supertonic)
- Intelligent backend selection based on text complexity
- Voice profile management
- Prosody and affective modulation (emotion-aware speech)
- UMN signal coordination for TTS events

Backends:
- smart: SmartTTSRouter with Supertonic (normalization) + Piper (clean text)
- supertonic: Supertonic only (built-in normalization, 167x real-time)
- piper: Piper only (fast for clean prose)
- mock: Mock backend for testing

UMN Signals:
- Emits: VOICE.TTS.AUDIO.READY (file path, duration, affective markers)
- Emits: VOICE.TTS.PLAY.AUDIO (file path to trigger playback via Audio I/O service)
...
class TTSService:
    """Text-to-speech service for speech synthesis."""

    def __init__(self):
        self.service_name = "kloros-voice-tts"
        ...
        self.tts_backend_name = os.getenv("KLR_TTS_BACKEND", "smart")
        ...
        self.prewarm_backends = int(os.getenv("KLR_TTS_PREWARM", "1"))
    ...
    def start(self):
        ...
        print(f"[tts] Starting {self.service_name}")
        ...
        print(f"[tts] {self.service_name} ready and listening")

    def _init_tts_backend(self) -> None:
        """Initialize TTS backend based on configuration."""
        try:
            from src.voice.tts.base import create_tts_backend

            kwargs = {"out_dir": self.tts_out_dir}
            if self.tts_backend_name == "smart":
                kwargs["prewarm"] = bool(self.prewarm_backends)

            self.tts_backend = create_tts_backend(self.tts_backend_name, **kwargs)

            if self.tts_backend_name == "smart" and hasattr(self.tts_backend, "get_status"):
                status = self.tts_backend.get_status()
                print(f"[tts] ✅ SmartTTSRouter initialized: {status}")
            else:
    ...
    def _subscribe_to_signals(self):
        """Subscribe to UMN signals for TTS synthesis requests."""
        self.speak_sub = UMNSub(
    ...
    def shutdown(self):
        """Graceful shutdown of TTS service."""
        print(f"[tts] Shutting down {self.service_name}")
        self.running = False
        ...
        try:
            ...
            print(f"[tts] Error closing UMN connections: {e}")

        print(f"[tts] {self.service_name} shutdown complete")


def main():
    """Main entry point for TTS service daemon."""
    print("[tts] Starting KLoROS Voice Service")
