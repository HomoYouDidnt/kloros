#!/usr/bin/env python3
"""KLoROS Voice LLM Integration Service - Unified LLM adapter with remote/local fallback.
...
class LLMService:
    """LLM Integration service with unified adapter for remote/local backends."""

    def __init__(self):
        self.service_name = "kloros-voice-llm"
        ...
        self.llama_model = os.getenv("KLR_LLAMA_MODEL", "main")
        self.llama_url = os.getenv("KLR_LLAMA_URL", "http://localhost:8080")
        self.llama_api_endpoint = f"{self.llama_url}/completion"
        ...
        print(f"[llm] Initialized: enable={self.enable_llm}, remote_model={self.remote_llm_model}, local_model={self.llama_model}")

    def start(self):
        """Start the service and subscribe to UMN signals."""
        print(f"[llm] Starting {self.service_name}")
        ...
        available_backends.append("llama")
        ...
        self.chem_pub.emit(
            ...
            facts={
                ...
                "default_model": self.remote_llm_model if self.remote_llm_enabled else self.llama_model,
                "remote_enabled": self.remote_llm_enabled,
                "local_model": self.llama_model,
            }
        )

        print(f"[llm] {self.service_name} ready (backends: {', '.join(available_backends)})")
    ...
    def _subscribe_to_signals(self):
        """Subscribe to UMN signals for LLM requests."""
        self.llm_request_sub = UMNSub(
    ...
    def _on_llm_request(self, event: dict):
        ...
        try:
            ...
            for attempt in range(self.max_retries + 1):
                ...
                try:
                    if mode == "streaming":
                        resp = self._query_llama_streaming(
                            prompt, temperature=temperature, max_tokens=max_tokens,
                            model=model_override or self.llama_model
                        )
                    else:
                        resp = self._query_llama(
                            prompt, temperature=temperature, max_tokens=max_tokens,
                            model=model_override or self.llama_model
                        )

                    if not resp.startswith("Error:") and not resp.startswith("llama.cpp error:"):
                        response = resp
                        used_backend = "llama"
                        used_model = model_override or self.llama_model
                        self.stats["local_requests"] += 1
                        break
                    else:
                        error = resp
                        print(f"[llm] llama.cpp attempt {attempt_count} failed: {resp}")

                except Exception as e:
                    error = f"llama.cpp exception: {e}"
                    print(f"[llm] llama.cpp attempt {attempt_count} exception: {e}")
    ...
    def _query_llama(
        ...
        """Query local llama.cpp (non-streaming).

        Args:
            prompt: Input prompt
            temperature: Generation temperature
            max_tokens: Max tokens to generate (mapped to num_predict)
            model: Model to use (default: self.llama_model)
        ...
        try:
            payload = {
                "model": model or self.llama_model,
                "prompt": prompt,
                "stream": False,
                "options": {
                    "temperature": temperature,
            ...
            if r.status_code == 200:
                ...
                return f"Error: llama.cpp HTTP {r.status_code}"

        except requests.Timeout:
            return f"llama.cpp error: timeout ({self.local_timeout}s)"
        except requests.RequestException as e:
            return f"llama.cpp error: {e}"

    def _query_llama_streaming(
        self, prompt: str, temperature: float = 0.8, max_tokens: Optional[int] = None,
        model: str = None
    ) -> str:
        """Query local llama.cpp (streaming).
    ...
    def shutdown(self):
        """Graceful shutdown of LLM service."""
        print(f"[llm] Shutting down {self.service_name}")
        self.running = False
        ...
        print(f"[llm] {self.service_name} shutdown complete")


def main():
    """Main entry point for LLM service daemon."""
    print("[llm] Starting KLoROS Voice Service")
