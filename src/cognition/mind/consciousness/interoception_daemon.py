...
from src.cognition.mind.consciousness.interoception import InteroceptiveMonitor
from src.cognition.mind.consciousness.appraisal import AppraisalSystem
...
STUCK_PROCESS_THRESHOLD = 3    # Alert when more than 3 processes stuck in D state
...
class InteroceptionDaemon:
    ...
    def __init__(self):
        ...
        self.last_duplicate_process_emission = 0
        self.last_stuck_process_emission = 0
        ...
        self.lobotomy_initiated_sub = UMNSub(
        ...
        self.lobotomy_restored_sub = UMNSub(
    ...
    def check_process_health(self):
        """
        Check for process health issues and emit affective signals.

        Detects:
        1. Duplicate kloros_voice processes (should only be one)
        2. Processes stuck in D state (uninterruptible disk sleep)

        Emits AFFECT_RESOURCE_STRAIN signals with appropriate strain_type.
        """
            ...
            if len(kloros_procs) > 1:
                if current_time - self.last_duplicate_process_emission > self.emission_cooldown:
                    ...
                    self.chem_pub.emit(
                        ...
                        intensity=1.5,
                        facts={
                            "strain_type": "duplicate_process",
                            "process_count": len(kloros_procs),
                            "pids": [p.pid for p in kloros_procs],
                            "autonomous_actions": ["kill_duplicate_processes"]
                        }
                    )
                    self.last_duplicate_process_emission = current_time

            if stuck_count > STUCK_PROCESS_THRESHOLD:
                if current_time - self.last_stuck_process_emission > self.emission_cooldown:
                    logger.warning(f"[interoception_daemon] Found {stuck_count} processes stuck in D state (threshold: {STUCK_PROCESS_THRESHOLD})")
                    ...
                    self.chem_pub.emit(
                        ...
                        intensity=1.5,
                        facts={
                            "strain_type": "stuck_processes",
                            "stuck_count": stuck_count,
                            "pids": stuck_pids[:10],
                            "autonomous_actions": ["investigate_io_issues"]
                        }
                    )
                    self.last_stuck_process_emission = current_time

        except Exception as e:
            logger.error(f"[interoception_daemon] Process health check failed: {e}", exc_info=True)
    ...
    def get_investigation_metrics(self) -> Dict[str, Any]:
        """Get investigation success/failure rates from UMN history."""
        kloros_home = Path('/home/kloros')
        history_file = kloros_home / '.kloros/umn_history.jsonl'
        ...
        try:
            ...
            logger.warning(f"[interoception_daemon] Failed to read UMN history: {e}")
            return {'success_rate': 0.5, 'failure_rate': 0.0, 'timeout_rate': 0.0}
    ...
    def compute_and_emit_affect_state(self):
        """Compute current affect state and emit it via UMN."""
        current_time = time.time()
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Check process health (duplicate/stuck processes)
