...
Analyzes METRICS_SUMMARY signals from UMN to track daemon resource usage,
...
class ResourceProfilerScanner(CapabilityScanner):
    ...
    def scan(self) -> List[CapabilityGap]:
        """Scan for resource usage issues using UMN daemon metrics."""
        gaps = []
        ...
        try:
            ...
            if self.chem_pub is None:
                try:
                    ...
                    logger.warning(f"[resource_profiler] Failed to initialize UMNPub: {e}")
    ...
    def _scan_umn_history(self) -> List[Dict[str, Any]]:
        """Scan UMN history for daemon resource usage patterns."""
        kloros_home = os.environ.get('KLOROS_HOME', '/home/kloros')
