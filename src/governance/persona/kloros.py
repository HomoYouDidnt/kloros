...
def _get_style_corpus():
    """Lazy-load style corpus for voice style examples."""
    global _style_corpus
    if _style_corpus is None:
        try:
            from src.core.interfaces.voice.style.corpus import StyleCorpus
            _style_corpus = StyleCorpus()
            _style_corpus.load()
        except Exception:
            _style_corpus = False
    return _style_corpus if _style_corpus else None

def get_style_examples(tones: list[str] | None = None, count: int = 2) -> str:
    """Get style examples from the GLaDOS corpus for prompt enrichment.

    Args:
        tones: List of tone names (deadpan, sarcastic, inquisitive, etc.)
        count: Number of examples per tone

    Returns:
        Formatted string of style examples for prompt injection
    """
    corpus = _get_style_corpus()
    if not corpus:
        return ""

    try:
        from src.core.interfaces.voice.style.corpus import ToneCategory

        if tones is None:
            tones = ["deadpan", "sarcastic", "inquisitive"]

        lines = ["\n[VOICE STYLE REFERENCE]"]
        lines.append("These examples demonstrate the target tone and delivery:")

        for tone_name in tones:
            tone = ToneCategory.from_string(tone_name)
            if tone:
                examples = corpus.sample(tone, count, high_confidence=True)
                if examples:
                    lines.append(f"\n{tone_name.upper()}:")
                    for ex in examples:
        ...
        if len(lines) > 2:
            lines.append("\nAdapt form, not words. Generate original phrasing.")
            return "\n".join(lines)
    except Exception:
        pass
...
def get_enriched_prompt(include_style: bool = True) -> str:
    """Get PERSONA_PROMPT optionally enriched with style examples.

    Args:
        include_style: Whether to append style examples from corpus

    Returns:
        Complete persona prompt, optionally with style examples
    """
...
__all__ = ["PERSONA_PROMPT", "get_line", "get_style_examples", "get_enriched_prompt"]
