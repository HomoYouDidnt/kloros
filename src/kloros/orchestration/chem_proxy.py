#!/usr/bin/env python3
# chem_proxy.py — XPUB/XSUB forwarder with verbose sub logging and TCP/IPC env toggles
import os
import sys
import time
...
XSUB_ENDPOINT = os.getenv("KLR_CHEM_XSUB", "tcp://127.0.0.1:5556")  # publishers CONNECT here
XPUB_ENDPOINT = os.getenv("KLR_CHEM_XPUB", "tcp://127.0.0.1:5557")  # subscribers CONNECT here
...
def main():
    ...
    print(f"[chem-proxy] XSUB bind @ {XSUB_ENDPOINT}  |  XPUB bind @ {XPUB_ENDPOINT}", flush=True)
        ...
        for sock, _ in poller.poll(timeout=1000):  # 1 second timeout for maintenance checks
            if sock is xpub_monitor:
                ...
                if ev == zmq.EVENT_ACCEPTED or ev == zmq.EVENT_CONNECTED:
                    print(f"[chem-proxy] XPUB peer connected", flush=True)
                elif ev == zmq.EVENT_DISCONNECTED:
                    print(f"[chem-proxy] XPUB peer disconnected", flush=True)
            elif sock is xsub_monitor:
                evt = mon.recv_monitor_message(sock)
                ev = evt['event']
                if ev == zmq.EVENT_ACCEPTED or ev == zmq.EVENT_CONNECTED:
                    print(f"[chem-proxy] XSUB peer connected", flush=True)
                elif ev == zmq.EVENT_DISCONNECTED:
                    print(f"[chem-proxy] XSUB peer disconnected", flush=True)
            elif sock is xpub:
                ...
                if data:
                    ...
                    print(f"[chem-proxy] {action} '{topic}'", flush=True)
                    # Forward to XSUB so publishers can (optionally) filter upstream
                    xsub.send(data)
            elif sock is xsub:
                # Forward published multipart messages to subscribers
                print(f"[chem-proxy] XSUB socket has data!", flush=True)
                msg = xsub.recv_multipart()
                # msg[0] must be topic frame; leave as-is
                print(f"[chem-proxy] MSG: topic={msg[0][:50] if msg else b''}, parts={len(msg)}", flush=True)
                xpub.send_multipart(msg)
                print(f"[chem-proxy] MSG forwarded to XPUB", flush=True)
