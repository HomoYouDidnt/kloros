...
try:
    from .signal_router_v2 import SignalRouter
    SIGNAL_ROUTER_AVAILABLE = True
except ImportError:
    SIGNAL_ROUTER_AVAILABLE = False
    logger.warning("SignalRouter not available, using legacy intent routing only")

# SAFETY: ResourceGovernor for SPICA spawn protection
try:
    from src.governance.resource_governor import ResourceGovernor
    SAFETY_ENABLED = True
except ImportError:
    SAFETY_ENABLED = False
    logging.warning("ResourceGovernor not available, SPICA safety checks disabled")
...
# Module-level signal router (lazy init)
_signal_router: Optional['SignalRouter'] = None

def _get_signal_router() -> Optional['SignalRouter']:
    """Get or create the signal router instance."""
    global _signal_router
    if not SIGNAL_ROUTER_AVAILABLE:
        return None
...
def _try_chemical_routing(intent_type: str, intent: dict, incident_id: Optional[str] = None) -> bool:
    """
    Attempt to route intent via chemical signal bus.

    Returns:
        True if routed successfully via chemical signals
        False if should fall back to legacy processing
    """
    router = _get_signal_router()
    if not router:
        return False

    # Extract facts from intent data
    facts = intent.get("data", {})

    # Generate incident_id if not provided
    if not incident_id:
        incident_id = f"intent-{intent_type}-{int(datetime.now().timestamp())}"

    # Map legacy intent types to new intent types expected by SignalRouter
    # The router expects queue.* style intents, but coordinator uses underscore style
    intent_mapping = {
        "queue_latency_spike": "queue.latency_spike",
        "queue_stall": "queue.stall",
        "queue_congestion_forecast": "queue.congestion_forecast",
        "queue_orphaned": "queue.orphaned",
    }

    routable_intent = intent_mapping.get(intent_type, intent_type)

    try:
        routed = router.route_intent(
            routable_intent,
            intensity=1.0,
            facts=facts,
        ...
        if routed:
            logger.info(f"Intent {intent_type} routed via chemical signals (incident={incident_id})")
        else:
            logger.debug(f"Intent {intent_type} not routable via chemical signals, using legacy")

        return routed

    except Exception as e:
        logger.error(f"Chemical routing failed for {intent_type}: {e}", exc_info=True)
        return False
...
def _process_intent(intent_path: Path) -> str:
    ...
    try:
        ...
        # Handle different intent types (legacy routing)
        if intent_type == "trigger_phase_promotion_cluster":
            ...
            if SAFETY_ENABLED:
                try:
                    ...
                    if not can_spawn:
                        logger.error(f"[spica_spawn] Spawn blocked by ResourceGovernor: {reason}")
                        _archive_intent(intent_path, f"resource_blocked_{reason.split(':')[0]}")
                        return "SPICA_SPAWN_BLOCKED"
                except Exception as e:
                    logger.error(f"[spica_spawn] ResourceGovernor check failed: {e}")
                    # Continue with spawn if safety check fails (fail-open for now)
                    # TODO: Change to fail-closed once ResourceGovernor is proven stable
            ...
            # At autonomy level 3+, automatically execute diagnostics
            autonomy_level = int(os.getenv("KLR_AUTONOMY_LEVEL", "0"))
            if autonomy_level >= 3 and intent_type == "suggest_dream_diagnostic":
                logger.info(f"[autonomy={autonomy_level}] Auto-executing D-REAM diagnostic")

                try:
                    error_msg = intent.get("data", {}).get("message", "")
                    logger.info(f"Triggering D-REAM to diagnose: {error_msg}")

                    result = dream_trigger.run_once(
                        topic="remediation_diagnostic",
                        run_tag=f"auto_diagnostic_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    )

                    if result.exit_code == 0:
                        logger.info(f"[autonomy] D-REAM diagnostic completed successfully (generation={result.generation})")
                        _archive_intent(intent_path, "processed")
                        return "DIAGNOSTIC_EXECUTED"
                    else:
                        logger.warning(f"[autonomy] D-REAM diagnostic failed with exit code {result.exit_code}")
                        _archive_intent(intent_path, "processed")
                        return "DIAGNOSTIC_FAILED"

                except Exception as e:
                    logger.error(f"[autonomy] Failed to execute diagnostic: {e}")
                    _archive_intent(intent_path, "processed")
...
def tick() -> str:
    ...
    try:
        ...
        # ALWAYS generate curiosity questions first (drives discovery-to-integration loop)
        try:
            from src.registry.curiosity_core import CuriosityCore
            from src.registry.capability_evaluator import CapabilityMatrix

            # Generate fresh questions from capability analysis, module discovery, etc.
            core = CuriosityCore()
            matrix = CapabilityMatrix()  # This will scan capabilities and system state
            ...
            # Write questions to curiosity_feed.json for processing
            if feed.questions:
                with open(core.feed_path, 'w') as f:
                    json.dump(feed.to_dict(), f, indent=2)
                logger.info(f"CuriosityCore generated {len(feed.questions)} questions")
        except Exception as e:
            logger.error(f"CuriosityCore failed to generate questions: {e}")

        # ALWAYS process curiosity questions (independent of other actions)
