...
class CuriosityCoreConsumerDaemon:
    ...
    def _append_to_curiosity_feed(self, questions: list) -> None:
        """
        Emit questions via priority queues (or append to feed in legacy mode).
        ...
        try:
            ...
            if USE_PRIORITY_QUEUES and self.prioritizer:
                for question in questions:
                    self.prioritizer.prioritize_and_emit(question)
                logger.debug(f"[curiosity_core_consumer] Emitted {len(questions)} questions via prioritizer")
            else:
                CURIOSITY_FEED.parent.mkdir(parents=True, exist_ok=True)
                ...
                existing_ids = {q.get("id") for q in feed.get("questions", [])}
                ...
                for question in questions:
                    question_dict = {
                        ...
                        "evidence_hash": question.evidence_hash,
                        "action_class": question.action_class.value,
                    ...
                    if question_dict["id"] not in existing_ids:
                        feed["questions"].append(question_dict)

                with open(CURIOSITY_FEED, 'w') as f:
                    json.dump(feed, f, indent=2)

                logger.debug(f"[curiosity_core_consumer] Updated curiosity feed with {len(questions)} questions (legacy mode)")
    ...
    def _generate_all_questions(self) -> None:
        ...
        try:
            ...
            if questions:
                self._append_to_curiosity_feed(questions)
                self.question_count += len(questions)
                logger.info(f"[curiosity_core_consumer] Proactive generation: {len(questions)} questions")
            else:
