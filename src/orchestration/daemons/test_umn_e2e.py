#!/usr/bin/env python3
"""
End-to-end tests with REAL UMN (not mocked).

These tests use a separate UMN proxy on custom TCP ports to avoid
conflicts with the production proxy running on 5556/5557.

Run with: pytest test_umn_e2e.py -v -s
...
class TestUMNEndToEnd(unittest.TestCase):
    """
    End-to-end tests with real UMN proxy.

    These tests:
    1. Start a dedicated test UMN proxy on ports 5558/5559
    ...
    @classmethod
    def setUpClass(cls):
        """Start dedicated UMN proxy for testing."""
        ...
        os.environ['KLR_UMN_XSUB'] = cls.test_xsub
        os.environ['KLR_UMN_XPUB'] = cls.test_xpub
...
print("Test UMN proxy running on {cls.test_xsub} / {cls.test_xpub}", flush=True)
...
"""
        ...
        if cls.proxy_process.poll() is not None:
            raise RuntimeError("Test UMN proxy failed to start")
    ...
    def tearDownClass(cls):
        ...
        os.environ.pop('KLR_UMN_XSUB', None)
        os.environ.pop('KLR_UMN_XPUB', None)
    ...
    def test_integration_daemon_emits_to_real_umn(self):
        """Test that IntegrationMonitorDaemon emits questions to real UMN."""
        ...
        # Create daemon (will use test UMN via environment variables)
...
""")
        ...
        # Verify message was received through real UMN
        self.assertTrue(
            message_arrived,
            "No message received from UMN within timeout"
        )
    ...
    def test_exploration_daemon_emits_to_real_umn(self):
        """Test that ExplorationScannerDaemon emits questions to real UMN."""
        from src.orchestration.core.umn_bus import UMNSub
        ...
        # This test validates the UMN plumbing works, not detection logic
        if message_arrived:
            msg = self.received_messages[0]
            self.assertEqual(msg.get('signal'), 'curiosity.exploration_question')

    def test_message_delivery_latency(self):
        """Test that UMN message delivery latency is acceptable (<100ms)."""
