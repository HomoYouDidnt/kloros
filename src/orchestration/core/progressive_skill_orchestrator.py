...
from src.orchestration.core.skill_decomposer import SkillProgression, SkillLevel, get_skill_decomposer
...
def get_dream_config():
    ...
    logger.debug("[skill_orchestrator] D-REAM deprecated, using stub config")
    return _StubDreamConfig()

class ChamberBatchEvaluator:
    """Stub evaluator when D-REAM is deprecated."""
    def __init__(self, evaluator_class, init_kwargs):
        self.evaluator_class = evaluator_class
        self.init_kwargs = init_kwargs
        logger.warning("[skill_orchestrator] D-REAM deprecated - ChamberBatchEvaluator is a no-op")

    def evaluate_batch(self, candidates, context):
        logger.warning("[skill_orchestrator] D-REAM deprecated - returning empty results")
        return []
