...
from .umn_bus import UMNSub, UMNPub
...
class ZooidRuntime:
    ...
    def __init__(self, name: str, niche: str, topics: List[str], ipc_path: str = None):
        ...
        Args:
            ...
            ipc_path: Optional IPC path for UMN (default: uses UMN default)
        """
        self.name = name
        self.niche = niche
        self.kill = False
        ...
        # Chemical bus
        self.pub = UMNPub(ipc_path) if ipc_path else UMNPub()

        # Subscribe to target signals
        self._subs = [
            UMNSub(topic, self._on_wrapper, ipc_path, zooid_name=name, niche=niche)
            for topic in topics
        ]

        # Subscribe to kill switch
        self._gov_sub = UMNSub(
            "governance.kill",
            self._on_kill,
            ipc_path,
            zooid_name=name,
