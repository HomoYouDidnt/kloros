...
XSUB_ENDPOINT = os.getenv("KLR_UMN_XSUB", "tcp://127.0.0.1:5558")  # publishers CONNECT here
XPUB_ENDPOINT = os.getenv("KLR_UMN_XPUB", "tcp://127.0.0.1:5559")  # subscribers CONNECT here
...
def main():
    ...
    print(f"[umn-proxy-fixed] XSUB bind @ {XSUB_ENDPOINT}  |  XPUB bind @ {XPUB_ENDPOINT}", flush=True)
    print(f"[umn-proxy-fixed] Using built-in zmq.proxy() for reliable message delivery", flush=True)
    ...
    try:
        ...
        print("[umn-proxy-fixed] Shutting down", flush=True)
