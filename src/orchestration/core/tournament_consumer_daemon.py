...
from src.orchestration.core.umn_bus import _ZmqSub
...
class _StubChamberBatchEvaluator:
    """Stub evaluator for deprecated D-REAM system."""
    def __init__(self, evaluator_class, init_kwargs):
        self.evaluator_class = evaluator_class
        self.init_kwargs = init_kwargs
        logger.warning("[tournament_consumer] D-REAM deprecated - using stub evaluator")

    def evaluate_batch(self, candidates, context):
        logger.info(f"[tournament_consumer] Stub evaluator: would test {len(candidates)} candidates")
        return [], {"stub": True, "message": "D-REAM system deprecated"}
...
class TournamentConsumer:
    ...
    def _execute_tournament(self, question_data: Dict[str, Any]):
        ...
        try:
            ...
            from src.orchestration.core.chamber_mapper import get_chamber_mapper, get_dream_config
            import random
            ...
            # Wrap chamber evaluator for batch evaluation (D-REAM deprecated, using stub)
            evaluator = _StubChamberBatchEvaluator(evaluator_class, init_kwargs)
