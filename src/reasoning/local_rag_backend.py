...
FAST_PATHS = {
    ...
    "memory doing": "memory_status",
    "memory status": "memory_status",
    "chromadb": "memory_status",
    "sqlite": "memory_status",
    "chromadb and sqlite": "memory_status",
    "working as expected": "system_diagnostic",
...
def validate_llm_claims(response_text: str, tools_executed: list[str]) -> tuple[bool, str]:
    """Validate LLM response against actually executed tools.

    Detects fabricated claims like "optimized settings" or "20x speed-up" when no tools ran.
    This prevents HALLUCINATION 3 from the diagnostic report: LLM fabricating work it didn't do.
    ...
    Returns:
        ...
        - is_valid: True if no suspicious claims detected
        - warning_message: Empty if valid, otherwise describes the fabrication
    """
    ...
    suspicious_patterns = [
        (r'\d+x\s+(faster|speed|improvement|optimization)', 'Specific performance metrics (e.g., "20x faster")'),
        (r'optimized\s+(\w+\s+){0,3}settings?', 'Claims of optimization'),
        (r'analyzed\s+(\w+\s+){0,3}(task|settings|configuration)', 'Claims of analysis'),
        (r'improved\s+(\w+\s+){0,3}(quality|performance|speed)', 'Claims of improvements'),
        (r'achieved\s+(\w+\s+){0,3}(speed-up|improvement|optimization)', 'Claims of achievements'),
        (r'successfully\s+(configured|updated|modified|changed)', 'Claims of successful modifications'),
    ]
    ...
    if detected_claims and tools_count == 0:
        claims_str = ', '.join(detected_claims)
        warning = f"LLM fabrication detected: {claims_str} with ZERO tool executions"
...
class LocalRagBackend:
    ...
    def __init__(self, **kwargs):
        ...
        # Initialize conversation logger for episodic memory (Qdrant-based)
        self.conversation_logger = None
        self.episodic_qdrant_client = None
        try:
            from qdrant_client import QdrantClient
            from src.memory.qdrant_conversation_logger import QdrantConversationLogger

            # Initialize Qdrant client for episodic memory
            qdrant_url = os.getenv("QDRANT_URL", "http://localhost:6333")
            self.episodic_qdrant_client = QdrantClient(url=qdrant_url)

            # Create Qdrant-based logger instance
            self.conversation_logger = QdrantConversationLogger(
                client=self.episodic_qdrant_client,
                collection_prefix="kloros"
            )
            print("[rag] Episodic memory logger initialized (Qdrant)")
        except Exception as e:
        ...
        if enable_agentflow:
            try:
                ...
                from src.ace.qdrant_store import QdrantBulletStore
                ...
                # Initialize Qdrant client for ACE bullets
                if HAS_QDRANT:
                    ...
                    if server_url is None:
                        try:
                            ...
                            if config_path.exists():
                                ...
                                server_url = toml_config.get("vector_store", {}).get("server_url", None)
                        except Exception:
                            pass

                    if server_url:
                        self.qdrant_client = QdrantClient(url=server_url)
                    else:
                        qdrant_dir = os.getenv('KLOROS_QDRANT_DIR', '/home/kloros/.kloros/qdrant_data')
                        os.makedirs(qdrant_dir, exist_ok=True)
                        self.qdrant_client = QdrantClient(path=qdrant_dir)

                    # Initialize ACE bullet store
                    self.ace_store = QdrantBulletStore(self.qdrant_client, collection_name="ace_bullets")
                    ...
                    print("[agentflow] AgentFlow + ACE initialized (Qdrant)")
                else:
                    print("[ace] qdrant-client not installed, ACE bullets unavailable")
                    self.ace_store = None
            except Exception as e:
    ...
    def _reformulate_for_tts(self, deepseek_response: str, original_query: str) -> str:
        """Reformulate DeepSeek reasoning into concise TTS-appropriate response.

        Takes DeepSeek-R1's verbose reasoning and passes it to the LIVE model
        to generate a persona-appropriate, concise response for voice output.

        Args:
            deepseek_response: Full response from DeepSeek including <think> tags
            original_query: The original user query

        Returns:
            Concise, persona-appropriate response suitable for TTS
        """
        try:
            # Extract reasoning from <think> tags
            think_match = re.search(r'<think>(.*?)</think>', deepseek_response, re.DOTALL)
            reasoning = think_match.group(1).strip() if think_match else ""

            # Extract the answer (everything after </think>)
            answer_match = re.search(r'</think>\s*(.*)', deepseek_response, re.DOTALL)
            answer = answer_match.group(1).strip() if answer_match else deepseek_response

            if not reasoning:
                # No <think> tags found, return as-is but trimmed
                print("[think→live] No reasoning tags found, using response as-is")
                return answer if answer else deepseek_response.strip()

            print(f"[think→live] Reformulating {len(reasoning)} chars of reasoning into TTS")

            # Load LIVE model config
            from src.config.models_config import get_ollama_model_for_mode, get_ollama_url_for_mode
            live_model = get_ollama_model_for_mode("live")
            live_url = get_ollama_url_for_mode("live")

            # Build reformulation prompt with persona
            from src.persona.kloros import PERSONA_PROMPT
            reformulation_prompt = f"""{PERSONA_PROMPT}

User asked: {original_query}

DeepSeek analyzed this and concluded:
{answer}

With this reasoning:
{reasoning[:500]}...

Provide a concise (1-2 sentence) voice-appropriate response that captures the essence of this analysis while staying in character. No internal reasoning - just the final answer."""

            # Call LIVE model via /api/generate
            import requests
            ...
            print(f"[think→live] Reformulated to {len(reformulated)} chars")

            return reformulated if reformulated else answer

        except Exception as e:
            print(f"[think→live] Reformulation failed: {e}, falling back to answer")
            # Fallback: strip <think> tags and return answer
    ...
    def _get_embedder_function(self):
        ...
        try:
            ...
            from src.config.models_config import get_embedder_model, get_embedder_fallbacks, get_embedder_trust_remote_code
            import torch
            ...
            # Force CPU for embeddings to preserve GPU VRAM for LLM inference
            # Embeddings are infrequent; CPU has sufficient capacity
            device = 'cpu'
            print(f"[semantic] Using CPU for embedder to preserve GPU memory for LLM")

            for model_name in models:
                try:
                    self._embedder = SentenceTransformer(model_name, device=device, trust_remote_code=trust_remote_code)
                    self._embedder_func = lambda text: self._embedder.encode(text)
                    print(f"[semantic] Using SentenceTransformer embedder: {model_name} on {device}")
                    return self._embedder_func
                except RuntimeError as e:
                    if 'CUDA out of memory' in str(e) and device == 'cuda':
                        # Retry on CPU if GPU is full
                        print(f"[semantic] GPU full, retrying {model_name} on CPU")
                        try:
                            self._embedder = SentenceTransformer(model_name, device='cpu', trust_remote_code=trust_remote_code)
                            self._embedder_func = lambda text: self._embedder.encode(text)
                            print(f"[semantic] Using SentenceTransformer embedder: {model_name} on CPU")
                            return self._embedder_func
                        except Exception as cpu_e:
                            print(f"[semantic] Failed to load {model_name} on CPU: {cpu_e}")
                            continue
                    else:
                        print(f"[semantic] Failed to load {model_name}: {e}")
                        continue
                except Exception as e:
                    print(f"[semantic] Failed to load {model_name}: {e}")
                    continue
    ...
    def _handle_validation_failure(
        ...
        if self.tool_synthesizer:
            ...
            if self.tool_synthesizer.capture_failed_tool_request(desired_tool_name, context=transcript):
                # Check for quota exceeded (chaos injection for testing)
                import os
                if os.getenv("KLR_FORCE_QUOTA_EXCEEDED") == "1":
                    print(f"[synthesis] 🚫 Quota exceeded (chaos injection)")

                    # Emit heal event for self-healing
                    try:
                        from src.self_heal.adapters.kloros_rag import emit_quota_exceeded
                        if self.heal_bus:
                            emit_quota_exceeded(self.heal_bus, desired_tool_name)
                    except ImportError:
                        print(f"[synthesis] ⚠️ Could not import emit_quota_exceeded")

                    if self.ack_broker:
                        self.ack_broker.maybe_ack("Rate limit reached. Waiting...")
                    return None
    ...
    def reply(self, transcript: str, kloros_instance=None, mode: str = None, suppress_streaming: bool = False, **kwargs) -> ReasoningResult:
        ...
        Args:
            ...
            suppress_streaming: If True, disable streaming TTS (for internal system prompts)
            **kwargs: Additional parameters (e.g., enable_xai) - currently unused but accepted for compatibility
        ...
        # Get model selection early (needed for fast-path formatting)
        ...
        # CRITICAL: Fast-path formatting must always use LIVE mode to avoid <think> tags in TTS
        fast_path_model_mode = "live"
        fast_path_model = get_ollama_model_for_mode("live")
        fast_path_url = get_ollama_url_for_mode("live")
        ...
        if matched_key:
            ...
            print(f"[fast-path] Matched '{transcript}' → {tool_name} (key: '{matched_key}')")

            # Execute tool quickly, then pass result through LLM for consistent formatting
            try:
                from src.introspection_tools import IntrospectionToolRegistry
                registry = IntrospectionToolRegistry()
                ...
                if tool_name in registry.tools:
                    ...
                    tool_result = tool.func(kloros_instance) if kloros_instance else tool.func()

                    print(f"[fast-path] Tool executed, passing result through LLM for formatting")

                    # Build prompt with PERSONA_PROMPT + tool result
                    from src.persona.kloros import PERSONA_PROMPT
                    tool_context = f"Tool '{tool_name}' returned:\n{tool_result}\n\nRespond to the user's question using this data."
                    fast_path_prompt = f"{PERSONA_PROMPT}\n\nUser: {transcript}\n\n{tool_context}\n\nAssistant:"

                    # Invoke LLM to format response (try remote first, then local)
                    try:
                        import requests
                        from src.config.models_config import check_remote_llm_available, query_remote_llm

                        # Try remote LLM first if available
                        remote_enabled, remote_model = check_remote_llm_available()
                        if remote_enabled:
                            success, response = query_remote_llm(fast_path_prompt, model=remote_model)
                            if success:
                                formatted_response = response.strip()
                                print(f"[fast-path] Formatted via remote LLM: {remote_model}")
                            else:
                                print(f"[fast-path] Remote LLM failed: {response}, trying local")
                                # Fall through to local ollama
                                raise Exception(f"Remote failed: {response}")
                        else:
                            # Remote not enabled, use local
                            raise Exception("Remote LLM not enabled")
                        ...
                        try:
                            ...
                            payload = {
                                ...
                                "options": {
                                    "temperature": 0.7,
                                    "num_predict": 200  # Allow room for formatting
                                }
                            }

                            api_response = requests.post(
                                f"{fast_path_url}/api/generate",
                                json=payload,
                                timeout=10
                            )
                            api_response.raise_for_status()
                            formatted_response = api_response.json().get("response", "").strip()

                            if not formatted_response:
                                formatted_response = tool_result  # Fallback to raw tool result

                        except Exception as llm_error:
                            print(f"[fast-path] LLM formatting failed: {llm_error}, using raw tool result")
                            formatted_response = tool_result  # Fallback to raw tool result

                    if xai_enabled:
                        xai_record = xai.finalize(
                            answer_summary=formatted_response,
                            citations=[f"Fast-path tool: {tool_name}"],
                            uncertainty_after=0.1,
                            rationale_outline=f"Fast-path: executed {tool_name}, formatted response via LLM with PERSONA_PROMPT"
                        )
                        self._log_xai_trace(xai_record)
                    ...
                    return ReasoningResult(
                        ...
                        meta={"backend": "local_rag", "fast_path": True, "tool": tool_name, "persona_applied": True}
                    )
            except Exception as e:
        ...
        try:
            ...
            # Model selection already done above for fast-path compatibility
            if model_mode != "live":
                print(f"[rag] Using {model_mode.upper()} mode: {selected_model} @ {selected_url}")

            # Handle nonsense/gibberish queries: invoke LLM to reason and clarify
            # Even if input seems unclear, KLoROS should reason about it and ask clarifying questions
            if query_type == "nonsense":
                print(f"[nonsense] Invoking LLM to reason about unclear input")

                # Build prompt with PERSONA_PROMPT + context about unclear input
                from src.persona.kloros import PERSONA_PROMPT
                clarity_context = (
                    "Note: This user input appears unclear or incomplete. "
                    "Reason about whether it's truly gibberish or if you can ask a clarifying question."
                )
                minimal_prompt = f"{PERSONA_PROMPT}\n\n{clarity_context}\n\nUser: {transcript}\n\nAssistant:"

                # Use fast model for low latency (try remote first, then local)
                try:
                    import requests
                    from src.config.models_config import check_remote_llm_available, query_remote_llm
                    ...
                    remote_enabled, remote_model = check_remote_llm_available()
                    if remote_enabled:
                        success, resp = query_remote_llm(minimal_prompt, model=remote_model)
                        if success:
                            response = resp.strip()
                            print(f"[nonsense] Handled via remote LLM: {remote_model}")
                        else:
                            raise Exception(f"Remote failed: {resp}")
                    else:
                        raise Exception("Remote LLM not enabled")
                    ...
                    try:
                        ...
                        payload = {
                            ...
                            "options": {
                                "temperature": 0.7,
                                "num_predict": 150  # Allow slightly longer for clarifying questions
                            }
                        }

                        api_response = requests.post(
                            f"{selected_url}/api/generate",
                            json=payload,
                            timeout=10
                        )
                        api_response.raise_for_status()
                        response = api_response.json().get("response", "").strip()

                        if not response:
                            response = "Unclear. Repeat that?"  # Fallback if LLM returns empty

                    except Exception as e:
                        print(f"[nonsense] LLM invocation failed: {e}, using fallback")
                        response = "Unclear. Repeat that?"  # Fallback on error
                ...
                if xai_enabled:
                    xai_record = xai.finalize(
                        ...
                        rationale_outline=f"Unclear input: invoked LLM with PERSONA_PROMPT to reason and clarify"
                    )
                    self._log_xai_trace(xai_record)
                ...
                return ReasoningResult(
                    reply_text=response,
                    sources=[],
                    meta={"backend": "local_rag", "query_type": "nonsense", "persona_applied": True, "reasoning_enabled": True}
                )

            # Handle conversational queries: invoke LLM with PERSONA_PROMPT but no tools/RAG
            # This ensures hallucination guards apply while preventing tool hallucination
            if query_type == "conversational":
                print(f"[conversational] Invoking LLM with PERSONA_PROMPT (no tools/RAG)")

                # Build minimal prompt with PERSONA_PROMPT only (no RAG context, no tools)
                from src.persona.kloros import PERSONA_PROMPT
                minimal_prompt = f"{PERSONA_PROMPT}\n\nUser: {transcript}\n\nAssistant:"
                ...
                try:
                    ...
                    if remote_enabled:
                        ...
                        if success:
                            response = resp.strip()
                            print(f"[conversational] Handled via remote LLM: {remote_model}")
                        else:
                            raise Exception(f"Remote failed: {resp}")
                    else:
                        raise Exception("Remote LLM not enabled")
                    ...
                    try:
                        ...
                        payload = {
                            ...
                            "options": {
                                "temperature": 0.7,
                                "num_predict": 100  # Keep responses brief
                            }
                        }
                        ...
                        if not response:
                            response = "Yes?"  # Fallback if LLM returns empty

                    except Exception as e:
                        print(f"[conversational] LLM invocation failed: {e}, using fallback")
                        response = "Yes?"  # Fallback on error
                ...
                if xai_enabled:
                    xai_record = xai.finalize(
                        ...
                        citations=[],
                        uncertainty_after=0.1,
                        rationale_outline=f"Conversational query: invoked LLM with PERSONA_PROMPT, no tools/RAG"
                    )
                    self._log_xai_trace(xai_record)
                ...
                return ReasoningResult(
                    reply_text=response,
                    sources=[],
                    meta={"backend": "local_rag", "query_type": "conversational", "persona_applied": True}
                )
            ...
            # NOTE: Streaming is also disabled for internal system prompts (suppress_streaming=True)
            stream_callback = None
            use_streaming = os.getenv("KLR_ENABLE_STREAMING_TTS", "0") == "1"
            if use_streaming and not suppress_streaming and kloros_instance and hasattr(kloros_instance, 'speak'):
                # State tracking for filtering in streaming mode
                stream_state = {'inside_think': False, 'buffer': '', 'tool_detected': False}

                def streaming_speak(sentence: str):
                    """Callback to speak sentences as they're generated.

                    Applies full filtering pipeline:
                    1. Filters <think>...</think> reasoning blocks (DeepSeek-R1)
                    2. Detects and blocks tool calls (TOOL: ...)
                    3. Filters consciousness introspection sections
                    """
                    from src.middleware import sanitize_output
                    from src.middleware.response_filter import detect_tool_call

                    # Accumulate into buffer to handle tags/patterns split across chunks
                    stream_state['buffer'] += sentence

                    # Check for tool calls - if detected, suppress ALL streaming output
                    if detect_tool_call(stream_state['buffer']):
                        if not stream_state['tool_detected']:
                            print(f"[streaming] Tool call detected, suppressing streaming TTS")
                            stream_state['tool_detected'] = True
                        return

                    output = []
                    remaining = stream_state['buffer']

                    # Filter <think> blocks
                    while remaining:
                        if not stream_state['inside_think']:
                            # Look for opening tag
                            open_match = re.search(r'<think>', remaining, re.IGNORECASE)
                            if open_match:
                                # Keep everything before the tag
                                before_tag = remaining[:open_match.start()]
                                if before_tag.strip():
                                    output.append(before_tag)
                                stream_state['inside_think'] = True
                                remaining = remaining[open_match.end():]
                            else:
                                # No opening tag found, keep everything
                                if remaining.strip():
                                    output.append(remaining)
                                remaining = ''
                        else:
                            # Inside block, look for closing tag
                            close_match = re.search(r'</think>', remaining, re.IGNORECASE)
                            if close_match:
                                # Skip everything inside the block
                                stream_state['inside_think'] = False
                                remaining = remaining[close_match.end():]
                            else:
                                # Still inside block, discard everything
                                remaining = ''

                    # Apply sanitization to filter introspection sections
                    if output:
                        text_to_speak = ''.join(output)
                        if text_to_speak.strip():
                            # Apply full sanitization (includes introspection filtering)
                            text_to_speak = sanitize_output(text_to_speak, aggressive=False)
                            if text_to_speak.strip():
                                print(f"[streaming] Speaking: {text_to_speak[:50]}...")
                                kloros_instance.speak(text_to_speak)

                    # Keep small buffer for potential pattern splits across chunks
                    stream_state['buffer'] = remaining[-20:] if len(remaining) > 20 else remaining
            ...
            if isinstance(rag_result, dict):
                ...
                # Reformulate DeepSeek reasoning through LIVE model for TTS
                if model_mode == "think":
                    response = self._reformulate_for_tts(response, transcript)
            ...
            # Emit heal event for self-healing
            if emit_rag_error and self.heal_bus:
                emit_rag_error(self.heal_bus, transcript, e)
    ...
    def reason(self, transcript: str, kloros_instance=None, mode: str = None, suppress_streaming: bool = False, **kwargs) -> ReasoningResult:
        """
        Backwards-compatible alias for reply().

        Used by legacy code paths that call .reason() instead of .reply().
        This shim prevents "LocalRagBackend object has no attribute 'reason'" errors.

        Args:
            transcript: User input text
            kloros_instance: Optional KLoROS instance for tool context
            mode: Optional mode override
            suppress_streaming: If True, disable streaming TTS (for internal system prompts)
            **kwargs: Additional parameters (e.g., enable_xai) passed to reply()

        Returns:
            ReasoningResult: Response with reply text, sources, and metadata
        """
        return self.reply(transcript, kloros_instance=kloros_instance, mode=mode, suppress_streaming=suppress_streaming, **kwargs)