...
def _announce_ollama_state_change(new_state: str):
    ...
    if message:
        try:
            # Non-blocking TTS via piper (GLaDOS voice)
            model_path = "/home/kloros/models/piper/glados_piper_medium.onnx"
            with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as tmp:
                # Generate speech with piper, then play with aplay
                subprocess.Popen(
                    f'echo "{message}" | piper -m {model_path} -f {tmp.name} && aplay -q {tmp.name} && rm {tmp.name}',
                    shell=True,
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL
                )
        except Exception:
            pass  # Fail silently if piper unavailable
