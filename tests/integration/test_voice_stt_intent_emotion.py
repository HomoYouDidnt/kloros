...
Tests the communication between STT, Intent, and Emotion zooids via real ChemBus.
No mocks for ChemBus - uses actual ChemBus pub/sub with real signal coordination.
"""
import os
import sys
import time
...
class TestOrchestratorSessionIntegration:
    ...
    Note: These tests verify basic ChemBus connectivity and signal reception.
    Due to ChemBus message echoing and shared bus architecture, exact message
    counts may vary. The unit tests (test_voice_session.py) provide comprehensive
    validation of Session zooid functionality with mocked ChemBus.
    """
    ...
    def test_session_stt_transcription_flow(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Verify message was added to history (allow for ChemBus echoing)
    ...
    def test_session_llm_response_flow(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Verify response was added to history (allow for ChemBus echoing)
                assert len(zooid.conversation_history) >= initial_count + 1
                # Find our message in the history
                found = any(
    ...
    def test_session_full_conversation_flow(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                # Verify conversation history (allow for ChemBus echoing)
