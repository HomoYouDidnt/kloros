...
Tests each zooid in isolation with mocked ChemBus and dependencies.
...
### Layer 2: Integration Tests (Real ChemBus, No Mocks)

Tests zooid pairs communicating via real ChemBus pub/sub.
...
### ChemBus Mock
Location: `tests/fixtures/chembus_mock.py`

Provides `MockChemPub` and `MockChemSub` for unit testing without real ChemBus.
...
- **Integration Tests**: All ChemBus signal flows validated
- **E2E Tests**: Basic conversation loop validated

## Known Limitations

1. Unit tests mock audio backends - real audio I/O tested separately
2. Integration tests may require ChemBus proxy to be running
...
def test_new_signal_flow(self, monkeypatch):
    ...
    # Subscribe to signal
    sub = ChemSub("SIGNAL.NAME", on_signal, ...)
...
### Tests fail with "ChemBus connection refused"
- Ensure ChemBus proxy is running for integration tests
