...
class MockChemPub:
    """Mock ChemBus publisher for testing."""
    def __init__(self):
        self.signals = []
...
class MockChemSub:
    """Mock ChemBus subscriber for testing."""
...
@pytest.fixture
def mock_chembus(monkeypatch):
    """Mock ChemBus for all tests."""
    mock_pub = MockChemPub()
    mock_sub_class = MockChemSub

    # Patch ChemPub/ChemSub where they're imported in kloros_voice_session
    monkeypatch.setattr("src.kloros_voice_session.ChemPub", lambda *args, **kwargs: mock_pub)
    monkeypatch.setattr("src.kloros_voice_session.ChemSub", mock_sub_class)
...
@pytest.fixture
def zooid(mock_chembus, temp_persist_dir, monkeypatch):
    ...
    # This ensures we use the mocked ChemPub that tracks signals
    return zooid
...
class TestSessionZooidStart:
    ...
    def test_start_emits_ready_signal(self, zooid, mock_chembus):
    ...
    def test_start_creates_subscriptions(self, zooid):
        """Test that start() creates ChemBus subscriptions."""
        zooid.start()
...
class TestSTTTranscriptionHandling:
    ...
    def test_on_stt_transcription_emits_session_updated(self, zooid, mock_chembus):
        """Test that STT transcription emits VOICE.SESSION.UPDATED signal."""
        zooid.start()
        mock_chembus["pub"].signals.clear()  # Clear ready signal
...
class TestLLMResponseHandling:
    ...
    def test_on_llm_response_emits_session_updated(self, zooid, mock_chembus):
        """Test that LLM response emits VOICE.SESSION.UPDATED signal."""
        zooid.start()
        mock_chembus["pub"].signals.clear()
...
class TestSessionMetadata:
    ...
    def test_emit_session_updated(self, zooid, mock_chembus):
...
class TestSessionZooidShutdown:
    ...
    def test_shutdown_emits_signal(self, zooid, mock_chembus):
