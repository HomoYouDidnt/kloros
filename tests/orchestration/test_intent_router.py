...
from src.kloros.orchestration.intent_router import IntentRouter
...
def test_intent_router_reads_discover_module_intent(temp_intent_dir, temp_dlq_file):
    ...
    with patch('src.kloros.orchestration.intent_router.ChemPub') as mock_pub_class:
        mock_pub = Mock()
...
def test_intent_router_deletes_processed_file(temp_intent_dir, temp_dlq_file):
    ...
    with patch('src.kloros.orchestration.intent_router.ChemPub'):
        router = IntentRouter(
            intent_dir=str(temp_intent_dir),
            dlq_path=str(temp_dlq_file)
        )
