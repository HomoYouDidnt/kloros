...
def test_emit_signal_publishes_to_chemical_bus():
    ...
    with patch('src.kloros.orchestration.orchestrator_monitor.ChemPub') as mock_pub_class:
        mock_pub = Mock()
        mock_pub_class.return_value = mock_pub

        monitor = OrchestratorMonitor(check_interval_s=60)
...
async def test_daemon_runs_continuously():
    ...
    with patch('src.kloros.orchestration.orchestrator_monitor.ChemPub'):
        monitor = OrchestratorMonitor(check_interval_s=0.05)
