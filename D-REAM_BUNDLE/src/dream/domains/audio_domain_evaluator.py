...
class AudioDomainEvaluator(DomainEvaluator):
    ...
    def _run_rtl_test(self) -> Dict[str, float]:
        ...
        try:
            # Use jack_iodelay or pw-loopback for testing
            if self.has_pipewire:
                # Create loopback and measure latency
                cmd = ['pw-loopback', '--capture-props=media.class=Audio/Source',
                ...
                # Get latency info from pw-top
                cmd = ['pw-top', '-b', '-n', '1']
                returncode, stdout, _ = self.run_command(cmd, timeout=2)

                if returncode == 0:
                    # Parse latency from output
                    # In production, would parse actual format
                    metrics['round_trip_ms'] = 10.5  # Placeholder
                ...
                metrics['round_trip_ms'] = 20.0  # Placeholder

        except Exception as e:
            logger.error(f"Failed to run RTL test: {e}")
            metrics['round_trip_ms'] = 50.0
